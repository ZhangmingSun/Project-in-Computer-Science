...
public class DatastoreServer
{
	...
	{
		ServerCommand ServerCmd = null;
		String CmdType = null;
		try {
			CmdType = StreamUtil.readLine(inputStream);	// The first line is the type of the command
			//logger.error(CommmdType + " is read from inputStream");	// It is using for debug
		} catch (IOException e) {
			e.printStackTrace();
			logger.error("Read inputStream occurs exceptions!");
		}
		
		if(CmdType.equalsIgnoreCase("write"))
			ServerCmd = new WriteCommand();
		else if(CmdType.equalsIgnoreCase("directory"))
			ServerCmd = new DirectoryCommand();
		else if(CmdType.equalsIgnoreCase("read"))
			ServerCmd = new ReadCommand();
		else if(CmdType.equalsIgnoreCase("delete"))
			ServerCmd = new DeleteCommand();
		else
			throw new ServerException("There is no such type of operation!");
